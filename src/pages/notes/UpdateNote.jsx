...
export default function UpdateNote() {
  const { showNote, getNote, isLoadingNote } = useShowNote();
  const title = useRef(null);
  const [newcategory, setNewcategory] = useState("");
  const [oldcategory, setOldcategory] = useState(showNote.category || "");
  const [isNote, setNote] = useState(showNote.note_content || "");
  const [selectedImages, setSelectedImages] = useState(showNote.images || []);
  ...
  const handlerFileUpload = useCallback(
    (event) => {
      ...
      const maxsimal =
        selectedImages.length + NewImages.length + selectedFilesArray.length;
      if (maxsimal > 8) {
      ...
      setNewImages((prev) => prev.concat(imagesWithPreview));
  ...
  const RemoveImageNewHandler = useCallback(
    (index) => {
      setNewImages(NewImages.filter((i) => i.preview !== index));
    },
    [NewImages]
  );
  ...
  useEffect(() => {
    if (showNote.length <= 0) {
      const getCategory = async () => {
        ...
        setOldcategory(response.category);
        setNote(response.note_content);
  ...
  const handlerSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const imageid = selectedImages.map((item) => item.id.toString());
            ...
            <div className="flex items-center w-full gap-x-3">
              ...
              <div className="md:w-3/12 w-1/2">
                ...
                <select
                  ...
                  // defaultValue={showNote.category}
                  value={oldcategory}
                  ...
                  return (
                    <div key={index} className="w-1/4 p-1 ">
                      <div className="rounded-md overflow-hidden w-full h-32 relative">
                        <button
                          type="button"
                          onClick={() => RemoveImageNewHandler(img.preview)}
                          className="absolute top-1 right-1 bg-white rounded-full p-[1px]"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
